...
const FilterBar = ({
  ...
  const [internalDateType, internalSetDateType] = useState("Ngày");
  const dateType = propDateType ?? internalDateType;
  const setDateType = propSetDateType ?? internalSetDateType;
  const dateTypes = ["Ngày", "Tuần", "Tháng"];
  ...
  function getCurrentWeek() {
    ...
    const pastDaysOfYear = (Number(now) - Number(firstDayOfYear)) / 86400000;
    const week = Math.ceil((pastDaysOfYear + firstDayOfYear.getDay() + 1) / 7);
    return `${year}-W${week.toString().padStart(2, "0")}`;
  }

  // Khi đổi loại dateType thì cập nhật lại selectedDate
        ...
        <div className="flex items-center gap-6 text-[#262626]">
          ...
          <div className="flex items-center gap-6 text-[#1F1F1F]">
            ...
            <div className="date-select">
              <select
                className="flex items-center gap-2 px-3 h-10 bg-[#FCFCFC] border border-[#D9D9D9] rounded-lg hover:border-[#1677FF] transition-all duration-200 cursor-pointer w-full text-[#1F1F1F]"
                value={dateType}
